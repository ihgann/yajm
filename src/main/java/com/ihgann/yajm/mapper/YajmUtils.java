...
public class YajmUtils {
  ...
  /**
   * Mapper function to map an object `U` to Class `V`. Attempts to infer based on field names of
   * the similar classes. If similar names are not found, checks for field names with {@link
   * Mapping} annotation from both classes to determine if there should be any linking done.
   *
  ...
  public static <V> V map(Object source, Class<V> destination)
    ...
    destinationFields.forEach(
        destField -> {
          ...
          // response object.
          if (destMappingAnnotation != null
              && destMappingAnnotation.other().equals(source.getClass())) {
            ...
            try {
              sourceValue =
                  ((MapperModifier) destMappingAnnotation.modifier().newInstance())
                      ...
                      sourceField -> {
                        ...
                        return sourceMappingAnnotation != null
                            ...
                            && ((destField.getName().equals(sourceField.getName())
                                ...
                                || (destField
                                    .getName()
                                    .equals(sourceMappingAnnotation.fieldName())));
                      })
          ...
          if (mappedSourceFields != null && mappedSourceFields.size() == 1) {
            ...
            try {
              destField.set(
                  response,
                  ((MapperModifier)
                          mappedField.getAnnotation(Mapping.class).modifier().newInstance())
          ...
          if (sameNameFields != null && sameNameFields.size() == 1) {
            ...
            try {
              destField.set(
                  response,
                  new DefaultMapperModifier().aToB(sameNameField.get(source), destField.getType()));
            } catch (IllegalAccessException e) {
              destField.setAccessible(destFieldOriginalAccessibility);
